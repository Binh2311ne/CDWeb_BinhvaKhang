...
const AdminDoctor = () =>{
  ...
  const mutation = useMutationHooks(
    (data) => {
      const{name, email, isAdmin, phone, password, confirmPassword, isDoctor} = data
      // Trả về dữ liệu nghĩa là mutation thành công isSuccess
      return UserService.signupUser({name, email, isAdmin : false,phone, password, confirmPassword, isDoctor: true})
    }
  )
  ...
  useEffect(()=>{
    if(isSuccess && data?.status ==='OK'){
      ...
      message.error('Có lỗi trong quá trình thêm nha sĩ')
    }
  },[isSuccess])
  ...
  return (
    <div>
      ...
      <ModalComponent forceRender title="Thêm nha sĩ" open={isModalOpen} onCancel={handleCancel} footer={null} >
        <Loading isPending={isPending}>
          ...
          >
            ...
            >
              <InputComponent type='password' value={stateUser.confirmPassword} onChange={handleOnChange} name="confirmPassword" />
            </Form.Item>
    ...
    >
      ...
      <DrawerComponent title="Chỉnh sửa thông tin nha sĩ" isOpen={isOpenDrawer} onClose={()=> setIsOpenDrawer(false)} width="70%">
